...
user function VaEstR08()
	...
	local nFreeze     := 0
	local nMax        := 999
	local cFieldOk    := "U_fFieldOK(oGetSin)" // "AllwaysTrue"
	local cSuperDel   := ""
	...
	Private oQtde     := NIL, cQtde := "1" + Space(10)

	SetKEY(VK_F5, {|| })

	aAdd(aHeadSin,{ " "				, "cStat"      		,"@BMP"         			,1,0,"","","C","","V","","","","V","","",""})
	//aAdd(aHeadSin,{ "Prod.Estoque"	, "B1_X_P RDES"		, X3Picture("B1_X_PRDES")	,TamSX3("B1_X_PRDES")[1]  , 0,"AllwaysTrue()", X3Uso("B1_X_PRDES"), "C", ""   , "V" } )
	aAdd(aHeadSin, { "Codigo"		, "B1_COD    "		, X3Picture("B1_COD    ")	, TamSX3("B1_COD    ")[1]/2  , 0, "AllwaysTrue()", X3Uso("B1_COD    "), "C", ""   , "V" } )
	aAdd(aHeadSin, { "Descrição" 	, "B1_DESC   "		, X3Picture("B1_DESC   ")	, TamSX3("B1_DESC   ")[1]/2-10  , 0, "AllwaysTrue()", X3Uso("B1_DESC   "), "C", ""   , "V" } )
	...
	aAdd(aHeadSin, { "Qtd Pesada"   , "VW_QTDPES "      , "@E 999999"               ,                         6  , 0, "AllwaysTrue()", X3Uso("B2_QATU")   , "N", "   ", "A" } )
	nUsadoSin := len(aHeadSin)

	beginSQL alias "QRY"
		%noParser%
		select B1_X_PRDES, B1_COD, B1_DESC, B1_UM, B1_LOCPAD, B1_GRUPO, B1_X_RUA, B1_X_MODUL, B1_X_NIVEL, B1_X_POSIC, 0 VW_QTDPES
		  from %table:SB1% sb1
	...
	nPosAux := Round(aPObjs[2,4]/5*4,0)+25
	...
	tSay():New(aPObjs[1,1]+12, aPObjs[1,2]+aPObjs[1,2]+45+200+70  ,{||'Qtd de Impressão:' },oGrp1,,/*oFont*/,,,,.T.,,,200,100)
...
/*---------------------------------------------------------------------------------,
 ...
 | Obs.     :                                                                      |
'---------------------------------------------------------------------------------*/
User Function fFieldOK(oGD)
Local nLinha  := oGD:oBrowse:nAt
Local nColuna := oGD:oBrowse:nColPos

	/* oGD:aCols[ nLinha, nColuna]  */
	If &(ReadVar()) > 0
		oGD:aCols[ nLinha, 1] := 'LBTIK'
	Else
		oGD:aCols[ nLinha, 1] := 'LBNO'
	EndIf
Return .T.
...
Local aAuxProds   := {}
Local nTotEtiq    := iIf(Empty(cQtde), 1, Val(cQtde))
Local nForTotEtiq := 0 // Iif(Empty(aProds[nY, 11]), nTotEtiq, aProds[nY, 11])

	For nX := 1 to len(aProds)
		nForTotEtiq := Iif(Empty(aProds[nX, 11]), nTotEtiq, aProds[nX, 11])
		For nY := 1 to nForTotEtiq
			aAdd( aAuxProds, aProds[nX] )
		Next nY
	Next nX
	aProds := aClone(aAuxProds)
