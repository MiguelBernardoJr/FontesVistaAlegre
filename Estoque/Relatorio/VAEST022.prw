...
oExcel:AddTable(  cWorkSheet, cTitulo)
	...
	/* 10 */ oExcel:AddColumn( cWorkSheet, cTitulo, "Quantidade"		     	 , 2, 2,.T. )
	/* 11 */ oExcel:AddColumn( cWorkSheet, cTitulo, "Custo"				     	 , 2, 3,.T. )
...
oExcel:AddTable(  cWorkSheet, cTitulo)
	...
	/* 04 */ oExcel:AddColumn( cWorkSheet, cTitulo, "Data Entrada"   	 	    , 1, 4 )
...
_cQry1 += "            CASE WHEN ZAB_EMERGE = 1 THEN 'SIM' ELSE 'NÃO' END AS EMERGENCIA,  " +CRLF
_cQry1 += " 	  CASE WHEN ZAB_OUTMOV = 1 THEN 'SIM' ELSE 'NÃO' END AS BOI_GORDO " +CRLF
_cQry1 += "   FROM "+RetSqlName("ZAB")+" ZAB " +CRLF
_cQry1 += "  WHERE ZAB_DTABAT BETWEEN '"+dToS(MV_PAR01-1)+"' AND '"+dToS(MV_PAR02)+"'   " + CRLF
...
Local cTitulo	 := "Saída do estoque por faturamento"
...
Local _cQry		 := ''
Local cAlias     := CriaTrab(,.F.)
Local cWorkSheet := "Devolução de Vendas"
...
// Devolução de Compra
Static Function fQuadro17(cPerg)
...
//Local nStdIni    := 0
Local nStdFim    := 0
Local dDataEst   := GetMV("MV_ULMES")//GetMV("MV_ULMES")
Local cAlias     := CriaTrab(,.F.)
Local cWorkSheet := "Saldo em Estoque"
Local cTitulo	 := "Saldo Em estoque por Filial - Custo Standard"

cTitulo += " - Dt. Referência: "  + DtoC(MV_PAR02)

oExcel:AddworkSheet(cWorkSheet)
oExcel:AddTable(  cWorkSheet, cTitulo)

_cQry += "   select B2_FILIAL, M0_FILIAL, B2_COD, B1_DESC,B2_LOCAL, B1_CUSTD" +CRLF
_cQry += "     from SB2010 SB2(nolock)" +CRLF
_cQry += "	 join SB1010 SB1 (nolock) ON" +CRLF
_cQry += "	      B1_COD = B2_COD" +CRLF
_cQry += "	  AND SB1.D_E_L_E_T_ = ' ' " +CRLF
_cQry += "	 join SYS_COMPANY SM0 on" +CRLF
...
_cQry += "  ORDER BY B2_FILIAL, B2_COD, B2_LOCAL" + CRLF
//_cQry += "    AND B2_FILIAL + B2_COD + B2_LOCAL  IN ( "+ CRLF
//_cQry += "         select D1_FILIAL+D1_COD + D1_LOCAL" + CRLF
//_cQry += "           FROM SD1010 SD1" + CRLF
//_cQry += "          WHERE D1_GRUPO IN ('BOV','01','05') " + CRLF
//_cQry += "            AND D1_DTDIGIT >= '"+DtoS(dDataEst)+"'" + CRLF
//_cQry += "            AND SD1.D_E_L_E_T_ = ' ' " + CRLF
//_cQry += "          UNION ALL " + CRLF
//_cQry += "         select D3_FILIAL+D3_COD + D3_LOCAL " + CRLF
//_cQry += "           FROM SD3010 SD3" + CRLF
//_cQry += "          WHERE D3_GRUPO IN ('BOV','01','05') " + CRLF
//_cQry += "            AND D3_EMISSAO >= '"+DtoS(dDataEst)+"'" + CRLF
//_cQry += "            AND SD3.D_E_L_E_T_ = ' ' " + CRLF
//_cQry += "          UNION ALL " + CRLF
//_cQry += "         select D2_FILIAL+D2_COD + D2_LOCAL" + CRLF
//_cQry += "           FROM SD2010 SD2" + CRLF
//_cQry += "          WHERE D2_GRUPO IN ('BOV','01','05') " + CRLF
//_cQry += "            AND D2_DTDIGIT >= '"+DtoS(dDataEst)+"'" + CRLF
//_cQry += "            AND SD2.D_E_L_E_T_ = ' ' " + CRLF
//_cQry += "	        union all " + CRLF
//_cQry += "		   select B9_FILIAL + B9_COD + B9_LOCAL " + CRLF
//_cQry += "		     from SB9010 SB9 " + CRLF
//_cQry += "		    where B9_QINI > 0 " + CRLF
//_cQry += "		      and B9_DATA ='"+DtoS(dDataEst)+"' " + CRLF
//_cQry += "		  	  and B9_COD LIKE 'BOV' " + CRLF
//_cQry += "		  	  and SB9.D_E_L_E_T_ = ' ' " + CRLF
//_cQry += "	        union all " + CRLF
//_cQry += "         select B2_FILIAL + B2_COD + B2_LOCAL" + CRLF
//_cQry += "  		 from SB2010 SB2 " + CRLF
//_cQry += "  		 join SB1010 SB1 on " + CRLF
//_cQry += "  		      B1_COD = B2_COD " + CRLF
//_cQry += "  		  and SB1.D_E_L_E_T_ = ' ' " + CRLF
//_cQry += "  	    where B2_QATU > 0 OR B2_QFIM > 0 " + CRLF
//_cQry += "  		  and B1_GRUPO IN ('01','05','BOV') " + CRLF
//_cQry += "  		  and SB2.D_E_L_E_T_ = ' ' " + CRLF
//_cQry += "     )" + CRLF
	...
	EndIf
		...
		MemoWrite(StrTran(cArquivo,".xml","")+"Quadro_Vendas.sql" , _cQry)

	dbUseArea(.T.,'TOPCONN',TCGENQRY(,, _cQry ),(cAlias),.F.,.F.) 
	
	//TcSetField(cAlias, "EMISSAO", "D")
	//TcSetField(cAlias, "DATA_ABATE", "D")
	//CalcEst( (cAlias)->B1_COD, (cAlias)->B2_LOCAL, sToD(dDtMoviment))[1] )
	While !(cAlias)->(Eof())         
		//nQuantIni := CalcEst( (cAlias)->B2_COD, (cAlias)->B2_LOCAL, (MV_PAR01)+1,(cAlias)->B2_FILIAL)[1]
		nQuantFim := (aAuxPrd := CalcEst( (cAlias)->B2_COD, (cAlias)->B2_LOCAL, (MV_PAR02),(cAlias)->B2_FILIAL))[1]
		//nStdIni := (cAlias)->B1_CUSTD * nQuantIni 
		nStdFim := (cAlias)->B1_CUSTD * nQuantFim 
		Aadd(aDados, { (cAlias)->B2_FILIAL, ;
		               (cAlias)->M0_FILIAL, ;
					   (cAlias)->B2_COD, ;
					   (cAlias)->B1_DESC, ;
					   (cAlias)->B2_LOCAL, ;
	...
	/* 01 */ oExcel:AddColumn( cWorkSheet, cTitulo, "Filial"		      		 	 , 1, 1 )
	/* 02 */ oExcel:AddColumn( cWorkSheet, cTitulo, "Filial Desc"		     	     , 1, 1 )
	/* 03 */ oExcel:AddColumn( cWorkSheet, cTitulo, "Codigo"		     	         , 1, 1 )
	/* 05 */ oExcel:AddColumn( cWorkSheet, cTitulo, "Era"		      		 	     , 1, 1 )
	/* 08 */ oExcel:AddColumn( cWorkSheet, cTitulo, "Armazém"		     	         , 1, 1 )
	///* 09 */ oExcel:AddColumn( cWorkSheet, cTitulo, "Qtd Ini "		     	         , 1, 2 )
	///* 10 */ oExcel:AddColumn( cWorkSheet, cTitulo, "Custo Std Ini"		     	     , 1, 2, .T. )
	/* 12 */ oExcel:AddColumn( cWorkSheet, cTitulo, "Qtd Fim"		     		     , 1, 2 )
	/* 12 */ oExcel:AddColumn( cWorkSheet, cTitulo, "Custo Std Fim"		     	     , 1, 2, .T. )
