...
/*/{Protheus.doc} VAJOB17
Rotina adaptada para execução em Schedule
...
User Function JOB17VA()
	local aMatrizFilial := {}
	local aDias := {}
	local aParams := ()
	Local nX := 0
	Local aFiliais := {}
	Local dDataIni := FirstDate(GetMv("MV_ULMES")) // DaySum(GetMv("MV_ULMES"),1)
	Local dDataFim := if(dDatabase >= LastDate(dDataIni), LastDate(dDataIni), dDataBase)
	Default lReproc := .T.

	aFiliais := ListFiliais() //{"05","12","13","20","21","23"} //
	aDias := GetDiasFromPeriodo(dDataIni, dDataFim)
	...
	for nX := 1 to len(aFiliais)
		...
		aMatrizFilial := U_GetMatrizSaldoErasPorDia(aFiliais[nX], dDataIni, dDataFim, lReproc, aParams) //}, "Processando", "Obtendo dados da Filial " + aFiliais[nX])
	Next
...
Local dDtFecha := GetMV("MV_ULMES")
Local dDataIni := FirstDate(dDia)//FirstDate(GetMv("MV_ULMES")) 
Local dDataFim := if(dDatabase >= LastDate(dDia), LastDate(dDia), dDataBase)
default lReproc := .F.
	...
	if nRecCalc == 0 .or. lReproc
		...
		/*
			dbUseArea(.T.,'TOPCONN',TCGENQRY(,,;
								cQry := "WITH BOV AS (" +CRLF +;
								        "select DISTINCT B2_COD, B2_LOCAL from  "+RetSqlName("SB2")+" SB2 where B2_FILIAL= '"+FwXFilial("B2")+"' and SB2.D_E_L_E_T_= ' ' and B2_COD in ( select B1_COD from  "+RetSqlName("SB1")+" SB1 where B1_FILIAL= '"+FwXFilial("SB1")+"'  and SB1.D_E_L_E_T_= ' ' and B1_X_ERA= '"+cEra+"' and B1_GRUPO='BOV' ) AND B2_QATU <> 0 AND B2_QFIM <> 0" +CRLF +;
								        "UNION" +CRLF +;
								        "SELECT DISTINCT D3_COD, D3_LOCAL FROM "+RetSqlName("SD3")+" WHERE D3_FILIAL = '"+FwXFilial("SD3")+"' AND D3_GRUPO = 'BOV' AND D3_EMISSAO BETWEEN '"+DToS(dDataIni)+"' AND '"+DToS(dDataFim)+"' AND D_E_L_E_T_ = ' ' " +CRLF +;
								        " UNION" +CRLF +;
								        "SELECT DISTINCT D1_COD, D1_LOCAL FROM "+RetSqlName("SD1")+" WHERE D1_FILIAL = '"+FwXFilial("D1")+"' AND D1_GRUPO = 'BOV' AND D1_DTDIGIT BETWEEN '"+DToS(dDataIni)+"' AND '"+DToS(dDataFim)+"' AND D_E_L_E_T_ = ' ' " +CRLF +;
								        " UNION" +CRLF +;
								        "SELECT DISTINCT D2_COD, D2_LOCAL FROM "+RetSqlName("SD2")+" WHERE D2_FILIAL = '"+FwXFilial("D2")+"' AND D2_GRUPO = 'BOV' AND D2_EMISSAO BETWEEN '"+DToS(dDataIni)+"' AND '"+DToS(dDataFim)+"' AND D_E_L_E_T_ = ' ' " +CRLF +;
								        "UNION " +CRLF +;
								        "SELECT DISTINCT  B9_COD, B9_LOCAL FROM "+RetSqlName("SB9")+" SB9 JOIN "+RetSqlName("SB1")+" SB1 ON B1_COD = B9_COD AND B1_GRUPO = 'BOV' AND SB1.D_E_L_E_T_ = ' ' WHERE B9_FILIAL = '"+FwXFilial("SB9")+"' AND B9_DATA ='"+DToS(dDtFecha)+"' AND SB9.D_E_L_E_T_ = ' ' AND B9_QINI <> 0" +CRLF +;
								        ")" +CRLF +;
								        "SELECT DISTINCT B2_COD, B2_LOCAL" +CRLF +;
								        "  FROM BOV " +CRLF +;
			...
			*/
			dbUseArea(.T.,'TOPCONN',TCGENQRY(,,;
								cQry := "WITH BOV AS (" +CRLF +;
								        "select DISTINCT B2_COD, B2_LOCAL from  "+RetSqlName("SB2")+" SB2 where B2_FILIAL= '"+cNumFilial+"' and SB2.D_E_L_E_T_= ' ' and B2_COD in ( select B1_COD from  "+RetSqlName("SB1")+" SB1 where B1_FILIAL= '"+FwXFilial("SB1")+"'  and SB1.D_E_L_E_T_= ' ' and B1_X_ERA= '"+cEra+"' and B1_GRUPO='BOV' ) AND (B2_QATU <> 0 OR B2_QFIM <> 0)" +CRLF +;
								        "UNION" +CRLF +;
								        "SELECT DISTINCT D3_COD, D3_LOCAL FROM "+RetSqlName("SD3")+" WHERE D3_FILIAL = '"+cNumFilial+"' AND D3_GRUPO = 'BOV' AND D3_EMISSAO BETWEEN '"+DToS(dDataIni)+"' AND '"+DToS(dDataFim)+"' AND D_E_L_E_T_ = ' ' " +CRLF +;
								        " UNION" +CRLF +;
								        "SELECT DISTINCT D1_COD, D1_LOCAL FROM "+RetSqlName("SD1")+" WHERE D1_FILIAL = '"+cNumFilial+"' AND D1_GRUPO = 'BOV' AND D1_DTDIGIT BETWEEN '"+DToS(dDataIni)+"' AND '"+DToS(dDataFim)+"' AND D_E_L_E_T_ = ' ' " +CRLF +;
								        " UNION" +CRLF +;
								        "SELECT DISTINCT D2_COD, D2_LOCAL FROM "+RetSqlName("SD2")+" WHERE D2_FILIAL = '"+cNumFilial+"' AND D2_GRUPO = 'BOV' AND D2_EMISSAO BETWEEN '"+DToS(dDataIni)+"' AND '"+DToS(dDataFim)+"' AND D_E_L_E_T_ = ' ' " +CRLF +;
								        "UNION " +CRLF +;
								        "SELECT DISTINCT  B9_COD, B9_LOCAL FROM "+RetSqlName("SB9")+" SB9 JOIN "+RetSqlName("SB1")+" SB1 ON B1_COD = B9_COD AND B1_GRUPO = 'BOV' AND SB1.D_E_L_E_T_ = ' ' WHERE B9_FILIAL = '"+cNumFilial+"' AND B9_DATA ='"+DToS(dDtFecha)+"' AND SB9.D_E_L_E_T_ = ' ' AND B9_QINI <> 0" +CRLF +;
								        ")" +CRLF +;
								        "SELECT DISTINCT B2_COD, B2_LOCAL" +CRLF +;
								        "  FROM BOV " +CRLF +;
								        "  JOIN "+RetSqlName("SB1")+" SB1 ON " +CRLF +;
		...
		while !TMPBOV->(Eof())
			nSaldo += CalcEst(TMPBOV->B2_COD,TMPBOV->B2_LOCAL, dDia)[1]
			TMPBOV->(dbSkip())
		endDo
		TMPBOV->(dbCloseArea())
...
Static Function ListFiliais()
Local aFiliais := {}
Local cQry := ""

	dbUseArea(.T.,'TOPCONN',TCGENQRY(,,;
								cQry := " SELECT DISTINCT B2_FILIAL " + CRLF +;
								        " from " +RetSqlName("SB2")+ " SB2 " + CRLF +;
								        " JOIN SB1010 SB1 ON B1_COD = B2_COD AND SB1.D_E_L_E_T_ = ' '  " + CRLF +;
								        " AND B1_GRUPO = 'BOV'  " + CRLF +;
								        " WHERE SB2.D_E_L_E_T_ = ' ' AND (B2_QFIM > 0 OR B2_QATU > 0) AND B2_FILIAL <> '"+FWxFilial("SB2")+"' " ;
										), "TMPFIL",.T.,.F.)
		While !(TMPFIL->(Eof()))
		aAdd(aFiliais, TMPFIL->B2_FILIAL)
		TMPFIL->(dbSkip())
		endDo
