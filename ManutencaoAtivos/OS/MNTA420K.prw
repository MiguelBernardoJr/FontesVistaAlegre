...
User Function MNTA420K()
    ...
    Local nPosTar  := aScan( aHeader, { |x| Trim( Upper( x[2] ) ) == 'TL_TAREFA' } )
    ...
    If nPosAlmx > 0 .And. nPosProd > 0 .And. nPosTpRg > 0 .And. nPos > 0 .AND. nPosNSa > 0
        ...
        If aCols[nPos,nPosTpRg] == 'P' .and. !Empty(aCols[nPos,nPosNSa])
            ...
            STL->(DbSeek(STJ->TJ_FILIAL+STJ->TJ_ORDEM+STJ->TJ_PLANO+aCols[nPos,nPosTar]+aCols[nPos,nPosTpRg]+aCols[nPos,nPosProd]))

            if aCols[nPos,nPosQtd] != STL->TL_QUANTID .or. aCols[nPos,nPosProd] != STL->TL_CODIGO 
                cQry := "SELECT * FROM "+RetSqlName("CB8")+" WHERE CB8_NUMSA = '"+aCols[nPos,nPosNSa]+"' AND CB8_ITEMSA = '"+aCols[nPos,nPosISa]+"' AND D_E_L_E_T_ = '' "
