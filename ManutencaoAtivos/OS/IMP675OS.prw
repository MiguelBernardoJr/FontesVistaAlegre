...
User Function IMP675OS()
    ...
    aAdd(aOpcRot, { "Retirada almoxarifado", "U_fOpcUser", { .F., "Ponto de Entrada 'fOpcUser'" } } )
...
/*/
//----------------------------------------------------------------------------------------
User Function fOpcUser()
    Local cQry := ''
    Local nRec := 0

    cQry := " select TJ_ORDEM,R_E_C_N_O_ AS REC   " + CRLF
    cQry += " from STJ010  " + CRLF
    cQry += " WHERE TJ_FILIAL = '"+STJ->TJ_FILIAL+"'  " + CRLF
    cQry += " AND TJ_ORDEM    = '"+STJ->TJ_ORDEM+"'   " + CRLF
    cQry += " AND TJ_PLANO    = '"+STJ->TJ_PLANO+"'   " + CRLF
    cQry += " AND TJ_TIPOOS   = '"+STJ->TJ_TIPOOS+"'  " + CRLF
    cQry += " AND TJ_CODBEM   = '"+STJ->TJ_CODBEM+"'  " + CRLF
    cQry += " AND TJ_SERVICO  = '"+STJ->TJ_SERVICO+"'  " + CRLF
    cQry += " AND TJ_SEQRELA  = '"+STJ->TJ_SEQRELA+"' " + CRLF
...
User Function MNTR675(lVPERG, cDEPLANO, cATEPLANO, aMATOS, nTipo, avMatSX1, nRecOs, cAls990 )
	...
	*/
	...
	If nTImpr = 2 .Or. nTImpr = 4
		...
		Else
 			Processa( { |lEnd| U_RW675IMP( oPrint, cDEPLANO, cATEPLANO, aMATOS, nRecOs, cAls990 ) }, STR0067 ) // "Aguarde... verificando alteracoes.."
		EndIf
...
User Function RW675Imp( oPrint, cDEPLANO, cATEPLANO, aMATOS, nRecOS, cAls990 )
	...
	local lIntPP	     := AllTrim(GetNewPar("MV_NGINTER","N")) == "M"
	Private aBenseP	 := {}
	...
	While !EoF()
		...
		If DbSeek( cBchSTJ + (cTRB675)->ORDEM + (cTRB675)->PLANO )
			...
			If STJ->TJ_TIPOOS == "B"
				...
				MNTW675Somal(oPrint)
				oPrint:Say(li,nHorz+15,"TESTE 1"+Space(1)+cPlaca,oFonTPN) //"Placa....:"
			...
			Else
				...
				MNTW675Somal(oPrint)
				oPrint:Say(li,nHorz+15,"TESTE 2"+Space(1)+cPlaca,oFonTPN) //"Placa....:"
			EndIf
			If !Empty(ST9->T9_LOCAL)
				...
				MNTW675Somal(oPrint)
				oPrint:Say(li,nHorz+15,"TESTE 3"+Space(1)+cPlaca,oFonTPN) //"Placa....:"
			EndIf
			...
			MNTW675Somal(oPrint)
				oPrint:Say(li,nHorz+15,"TESTE 4"+Space(1)+cPlaca,oFonTPN) //"Placa....:"
			...
			MNTW675Somal(oPrint)
				oPrint:Say(li,nHorz+15,"TESTE 5"+Space(1)+cPlaca,oFonTPN) //"Placa....:"

			// Impressao da Linha de Descrição (observação) do bem/veículo
			If mv_par03 == 1
				U_NGMEMORW675(STR0014,ST9->T9_DESCRIC,300,70,.F.) //"Descrição:"
				MNTW675Somal(oPrint)
			EndIf
			...
			For xk := 1 To Len(aARTAREFAS)
				...
				While &( cWhileTL ) .And. (cAlsSTL)->TL_TAREFA == aARTAREFAS[xk,1]
					...
					If lCabStl1
						...
						oPrint:Say(li,nHorz+1950,"Aplicado?",oFonTPN)	//Status.
					...
					oPrint:Say( li, nHorz+30, SubStr( TIPREGBRW((cAlsSTL)->TL_TIPOREG), 1, 4 ), oFonTPN ) //Nome

					oPrint:Say( li, nHorz+168, SubStr( (cAlsSTL)->TL_CODIGO, 1, 30 ), oFonTPN ) //Codigo
					oPrint:Say(li,nHorz+1240,SubStr( NOMINSBRW((cAlsSTL)->TL_TIPOREG, (cAlsSTL)->TL_CODIGO, (cAlsSTL)->TL_LOJA ),1,30),oFonTPN) //Descricao
					...
					If (cAlsSTL)->TL_TIPOREG != 'P'
						oPrint:Say( li, nHorz + 905, Str( U_MNT675CONV( (cAlsSTL)->TL_QUANTID, IIf( Empty( (cAlsSTL)->TL_TIPOHOR ), Nil, (cAlsSTL)->TL_TIPOHOR ) ), 9, 2 ), oFonTPN ) //Consumo
					Else
					...
					cQry := " SELECT *  " + CRLF 
					cQry += " FROM "+RetSQLName("STL")+"  " + CRLF 
					cQry += " WHERE TL_CODIGO = '"+(cAlsSTL)->TL_CODIGO+"'  " + CRLF 
					cQry += " AND TL_NUMSA 	= '"+(cAlsSTL)->TL_NUMSA+"' 	" + CRLF 
					cQry += " AND TL_ITEMSA = '"+(cAlsSTL)->TL_ITEMSA+"' 	" + CRLF 
					cQry += " AND TL_NUMSC 	= '"+(cAlsSTL)->TL_NUMSC+"' 	" + CRLF 
					cQry += " AND TL_ITEMSC = '"+(cAlsSTL)->TL_ITEMSC+"' 	" + CRLF 
					cQry += " AND TL_ORDEM 	= '"+(cAlsSTL)->TL_ORDEM+"'  	" + CRLF 
					...
					if !TMP->(EOF())
						...
						IF TMP->TL_TIPOREG == 'P' .and. !Empty( TMP->TL_NUMSA ) .And.;
                       		!Empty( TMP->TL_ITEMSA )

							nQuantid := TMP->TL_QUANTID
						ELSEIF lIntPP .and. (cAlsSTL)->TL_TIPOREG $ 'P\T' .And. !Empty( (cAlsSTL)->TL_NUMSC )  .And.;
							!Empty( (cAlsSTL)->TL_ITEMSC )

							// Considera a quantidade somente de insumos aplicados vinculados a mesma S.C.
							If nLine := ( Val(TMP->TL_NUMSC) > 0 .And.  Val(TMP->TL_ITEMSC) > 0 )
								nQuantid := TMP->TL_QUANTID
							EndIf
						else 

							cTiporeg := TMP->TL_TIPOREG
							cCodigo  := TMP->TL_CODIGO

							If AllTrim( (cAlsSTL)->TL_TAREFA ) == AllTrim( TMP->TL_TAREFA ) ;
								.And. ( ( Alltrim( (cAlsSTL)->TL_TIPOREG ) == "E" .And. AllTrim( cTiporeg ) == "M" ;
								.And. NGIFDBSEEK( "ST2", Padr( cCodigo, nSizeField )  + Alltrim( (cAlsSTL)->TL_CODIGO ), 1 ) ) ;
								.Or. ( AllTrim( (cAlsSTL)->TL_TIPOREG ) == AllTrim( cTiporeg ) ;
								.And. AllTrim( (cAlsSTL)->TL_CODIGO ) == AllTrim( cCodigo ) ) )
					...
					//IF (cAlsSTL)->TL_TIPOREG == 'P' .and. !Empty( (cAlsSTL)->TL_NUMSA ) .And.;
                    //    !Empty( (cAlsSTL)->TL_ITEMSA )
//
                    //    nLine := 0
//
                    //    // Considera a quantidade somente de insumos aplicados vinculados a mesma S.A.
                    //    If nLine := (!EMPTY((cAlsSTL)->TL_NUMSA) .and. !EMPTY((cAlsSTL)->TL_ITEMSA))
//
                    //        nQuantid := (cAlsSTL)->TL_QUANTID
                    //    ElseIf lIntPP .And. (cAlsSTL)->TL_TIPOREG $ 'P\T' .And.!Empty( (cAlsSTL)->TL_NUMSC )  .And.;
                    //            !Empty( (cAlsSTL)->TL_ITEMSC ) 
//
                    //            // Considera a quantidade somente de insumos aplicados vinculados a mesma S.C.
                    //            If nLine := ((cAlsSTL)->TL_NUMSC .And. (cAlsSTL)->TL_ITEMSC  > 0 )
//
                    //                nQuantid := (cAlsSTL)->TL_QUANTID
//
                    //            EndIf
...
//
                    ...
                    //            //----------------------------------------------------
                    //            If AllTrim( (cAlsSTL)->TL_TAREFA ) == AllTrim( aCoBrw6[ nIndex, nPTarefa ] ) ;
                    //                .And. ( ( Alltrim( (cAlsSTL)->TL_TIPOREG ) == "E" .And. AllTrim( cTiporeg ) == "M" ;
                    //                .And. NGIFDBSEEK( "ST2", Padr( cCodigo, nSizeField )  + Alltrim( (cAlsSTL)->TL_CODIGO ), 1 ) ) ;
                    //                .Or. ( AllTrim( (cAlsSTL)->TL_TIPOREG ) == AllTrim( cTiporeg ) ;
                    //                .And. AllTrim( (cAlsSTL)->TL_CODIGO ) == AllTrim( cCodigo ) ) )
...
//
                    ...
                    //IF (cAlsSTL)->TL_TIPOREG == 'P' .And. ( nNSA > 0 .And. !Empty( (cAlsSTL)->TL_NUMSA ) ) .And.;
                    //    ( nISA .And. !Empty( (cAlsSTL)->TL_ITEMSA ) )
//
                    //    nLine := 0
//
                    //    // Considera a quantidade somente de insumos aplicados vinculados a mesma S.A.
                    //    If nNSA > 0 .And. nISA > 0 .And. ( ( nLine := aScan( aCoBrw6, { |x| x[nNSA] == (cAlsSTL)->TL_NUMSA .And.;
                    //        x[nISA] == (cAlsSTL)->TL_ITEMSA } ) ) > 0 )
//
                    //        nQuantid := aCoBrw6[nLine,nPQuantid]
                    //    ElseIf lIntPP .And. (cAlsSTL)->TL_TIPOREG $ 'P\T' .And. ( nNSC > 0 .And. !Empty( (cAlsSTL)->TL_NUMSC ) ) .And.;
                    //            ( nISC .And. !Empty( (cAlsSTL)->TL_ITEMSC ) )
//
                    //            // Considera a quantidade somente de insumos aplicados vinculados a mesma S.C.
                    //            If nNSC > 0 .And. nISC > 0 .And. ( ( nLine := aScan( aCoBrw6, { |x| x[nNSC] == (cAlsSTL)->TL_NUMSC .And.;
                    //                x[nISC] == (cAlsSTL)->TL_ITEMSC } ) ) > 0 )
//
                    //                nQuantid := aCoBrw6[nLine,nPQuantid]
//
                    //            EndIf
...
//
                    ...
                    If nQuantid == 0
                        cStatus := "Não Aplicado"//"0" //'0' - Não aplicado
                    ElseIf nQuantid < (cAlsSTL)->TL_QUANTID
                        cStatus := "Parcialmente"//"1" //'1' - Parcialmente Aplicado
                    ElseIf nQuantid >= (cAlsSTL)->TL_QUANTID
                        cStatus := "Totalmente"//"2" //'2' - Totalmente aplicado
                    EndIf

                    oPrint:Say( li, nHorz + 1950, cStatus, oFonTPN ) // Status.

					MNTW675Somal(oPrint)

					If !Empty( (cAlsSTL)->TL_OBSERVA )

						U_NGMEMORW675( Space( 1 ) + STR0066, (cAlsSTL)->TL_OBSERVA, 470, 60, .T. )
						MNTW675Somal( oPrint )
...
User Function NGWIMPETAPA(cVOK,cVETAPA)
	If Empty(cVOK)
		...
		If dbSeek( cBchTPA + cVETAPA )
			U_NGMEMOEW675(TPA->TPA_DESCRI,150,70)
			If MV_PAR23 == 1
...
±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±±
±±ÚÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÂÄÄÄÄÄÄÂÄÄÄÄÄÄÄÄÄÄ¿±±
±±³ Fun‡…o   ³U_NGMEMOEW675³ Autor ³In cio Luiz Kolling    ³ Data ³29/05/2008³±±
±±ÃÄÄÄÄÄÄÄÄÄÄÅÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÄÁÄÄÄÄÄÄÁÄÄÄÄÄÄÄÄÄÄ´±±
...
@sample U_NGMEMORW675("Retorno","Descrição",1,2,.T.)
...
Static Function R675Imp( lEnd, wnRel, titulo, tamanho, cDEPLANO, cATEPLANO,;
	...
	While ( cTRB675 )->( !EoF() )
		...
		If dbSeek( cBchSTJ + (cTRB675)->ORDEM + (cTRB675)->PLANO )
			...
			U_SomaR765()		   		 // |Frota....:
			@ Li,000 Psay "|Frota....:" //"|C.Trab...:"
			IF !Empty(ST9->T9_FROTA) /* FROTA */
				U_SomaR765()		   		 // |Frota....:
				@ Li,000 Psay "|Frota....:" //"|C.Trab...:"
				@ Li,012 Psay SubStr( ST9->T9_FROTA,1,20 )
				@ Li,079 Psay "|"
			ENDIF 
			...
			For xk := 1 To Len(aARTAREFAS)
				...
				While &( cWhileTL ) .And. (cAlsSTL)->TL_TAREFA == aARTAREFAS[xk,1]
					...
					@ Li,001 Psay Substr(TIPREGBRW((cAlsSTL)->TL_TIPOREG),1,4)
					
					@ Li,006 Psay (cAlsSTL)->TL_CODIGO Picture '@!'
					@ Li,022+(nB1) Psay NOMINSBRW((cAlsSTL)->TL_TIPOREG, (cAlsSTL)->TL_CODIGO, (cAlsSTL)->TL_LOJA )
					@ Li,079 Psay "|"
					U_SomaR765()
					@ Li,000 Psay "|"
					...
					If (cAlsSTL)->TL_TIPOREG != 'P'
						@ Li,031 Psay U_MNT675CONV( (cAlsSTL)->TL_QUANTID, IIf( Empty( (cAlsSTL)->TL_TIPOHOR ), Nil, (cAlsSTL)->TL_TIPOHOR ) ) Picture '@E 999999.99' //Consumo
					Else
