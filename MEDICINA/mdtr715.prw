...
user Function INDMDT715( oPrint )
	...
	Local nAltura	:= 215 	//Altura do Logo
	Local nLargura 	:= 655 	//Largura do Logo
	...
	If lSigaMdtps
		...
		While !Eof() .And. TOC->TOC_Filial == xFilial( "TOC" ) .And. TOC->( TOC_CLIENT + TOC_LOJA ) <= cMVAteCli + cMVAteLoj
			...
			aAgeAssAti := U_MDT715AR( RetDtReferz() )
	...
	Else
		...
		While !Eof() .And. TOC->TOC_FILIAL == xFilial( "TOC" ) .And. &( cCodFu4 ) <= cMVAteFun
			...
			If FindFunction( "NGLocLogo" )
				If File( NGLocLogo() )
					oPrint:SayBitMap(105,105, NGLocLogo(), nLargura, nAltura )
				EndIf
			EndIf
			...
			ElseIf cMVDescAti == 2
				If SRJ->( FieldPos( "RJ_MEMOATI" ) ) > 0
					cMemo := RTrim( SRJ->RJ_MEMOATI )
				ElseIf !Empty( SRJ->RJ_REQMEMO)
					cMemo := SRJ->RJ_REQMEMO//RTrim( MSMM( SRJ->RJ_DESCREQ, , , , , , , , , "RDY" ) )
