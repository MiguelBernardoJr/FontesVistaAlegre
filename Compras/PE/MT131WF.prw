                ...
                while !TMPSC8->(Eof())
                    ...
                    if TMPSC8->C8_FILIAL+TMPSC8->C8_NUM+TMPSC8->C8_FORNECE+TMPSC8->C8_LOJA != cChave
                        ...
                        if lEnvia
                            ...
                            // cChave    :=    TMPSC8->C8_FILIAL+TMPSC8->C8_NUM+TMPSC8->C8_FORNECE+TMPSC8->C8_LOJA
                            while cChave == TMPSC8->C8_FILIAL+TMPSC8->C8_NUM+TMPSC8->C8_FORNECE+TMPSC8->C8_LOJA
                                /* Mb */
                                RecLock("SC1", .f.)
                                ...
                                if empty(SC8->C8_WFEMAIL)
                                    if cUsername == "Administrador"
                                        ...
                                        PswSeek(__cUserId)
                                        //cMailSend :=  "igor.oliveira@vistaalegre.agr.br"
                                        cMailSend := Iif(Empty(ALLTRIM( Posicione("SY1",3,xFilial("SY1")+__cUserId,"Y1_EMAIL") )),;
                                                                        PswRet(1)[1][14],;
...
u_EnvMail(cForMail,,, "Solicitação de proposta de cotação V@ " + SubStr(cChave, 1, TamSX3("C8_FILIAL")[1] + TamSX3("C8_NUM")[1]) + ".",;
