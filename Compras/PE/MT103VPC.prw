			...
			/* 
				funcao: NGASX7OP(), no fonte: MNTUTIL.PRX
					Iif(FindFunction("NGASX7OP"),NGASX7OP(),"")  
						-> A funcao abaixo, foi copida para preenchimento do campo Ordem de Servico;
			*/
			If Empty( aCols[val(cItem), nPosOS] )
				If SuperGetMV( 'MV_NGMNTNO', .F., '2' ) == '1' .And. ( SuperGetMV( 'MV_NGMNTES', .F., 'N' ) == 'S' .Or.;
					...
					If dbSeek( xFilial( 'SC2' ) + aCols[val(cItem), nPosOP] ) .And. SC2->( C2_ITEM + C2_SEQUEN ) == 'OS001';
							.And. ( 'OS001' $ aCols[val(cItem), nPosOP] .Or. '  001' $ aCols[val(cItem), nPosOP] )

						aCols[val(cItem), nPosOS] := SubStr( aCols[val(cItem), nPosOP], 1, /* nTamOS */Len( STJ->TJ_ORDEM ) )
