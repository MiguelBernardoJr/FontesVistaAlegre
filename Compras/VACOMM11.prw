...
User Function CanEdit(lCanEdit, lSupremo)
	...
	if cCampo == "ZBC_QUANT" .or. cCampo == "ZBC_VLRPTA" .or. cCampo == "ZBC_MARK" .or. cCampo == "ZBC_ALIICM"
		...
		lRet := Iif( Empty(o1ZBCGDad:aCols[o1ZBCGDad:nAt,nPZBCPed]),;
						.T.,;
						lSupremo .or. (lCanEdit .and. o1ZBCGDad:aCols[o1ZBCGDad:nAt,nPZBCPdP]=="P" ) ) // GERA PEDIDO POR 
	endif 
...
ElseIf cTipo == "oGetF4Q4E"
	...
	cSql += " SELECT B1_COD FROM " + RetSQLName('SB1') + " WHERE SUBSTRING(B1_XLOTCOM,8,6) IN (	 " + CRLF
	cSql += "  SELECT DISTINCT ZBC_PEDIDO FROM " + RetSQLName('ZBC') + " WHERE ZBC_FILIAL='" + M->ZCC_FILIAL + "' AND ZBC_CODIGO='" + M->ZCC_CODIGO + "' AND D_E_L_E_T_ = ' ' )" + CRLF
	cSql += " ) " + CRLF
...
ElseIf cTipo == "oGetF4Q1"
	...
	*/
	cSql += "	 WITH DADOS AS ( " + CRLF
	cSql += "     SELECT ZBC_PEDIDO [NUMERO_LOTE] " + CRLF
	cSql += "	      , SUM(ZBC_QUANT) [ZBC_COMPRA] " + CRLF
	cSql += "		  , ISNULL(( SELECT SUM(B8_SALDO)  " + CRLF
	cSql += "				       FROM " + RetSQLName('SB8') + "  " + CRLF
	cSql += "					  WHERE B8_FILIAL = '"+FWxFilial("SB8")+"'  " + CRLF
	cSql += "					    AND B8_PRODUTO IN ( " + CRLF
	cSql += "                                           SELECT B1_COD  " + CRLF
	cSql += "						  				     FROM " + RetSQLName('SB1') + "  " + CRLF
	cSql += "											WHERE SUBSTRING(B1_XLOTCOM,8,6) IN ( " +CRLF
	cSql += "										SELECT ZBC_PEDIDO FROM " + RetSQLName('ZBC') + " WHERE ZBC_CODIGO = '"+M->ZCC_CODIGO+"' AND D_E_L_E_T_ = ' ' )) " + CRLF
	cSql += "											),0) [SALDO_B8]								 " + CRLF
	cSql += "		  , ISNULL(( SELECT SUM(D2_QUANT)  " + CRLF
	cSql += "		               FROM " + RetSQLName('SD2') + " SD2 " + CRLF
	cSql += "					  WHERE D2_FILIAL = '"+FWxFilial("SD2")+"'  " + CRLF
	cSql += "					    AND D2_COD IN ( " + CRLF
	cSql += "						               SELECT B1_COD  " + CRLF
	cSql += "						  				 FROM " + RetSQLName('SB1') + "  " + CRLF
	cSql += "										WHERE SUBSTRING(B1_XLOTCOM,8,6) IN ( " +CRLF
	cSql += "										SELECT ZBC_PEDIDO FROM " + RetSQLName('ZBC') + " WHERE ZBC_CODIGO = '"+M->ZCC_CODIGO+"' AND D_E_L_E_T_ = ' ' )) " + CRLF
	cSql += "										),0) [FATURADO] " + CRLF
	cSql += "		  , ISNULL(( SELECT SUM(D3_QUANT)  " + CRLF
	cSql += "		               FROM " + RetSQLName('SD3') + "  " + CRLF
	cSql += "					  WHERE D3_FILIAL ='"+FWxFilial("SD3")+"' " + CRLF
	cSql += "					    AND D3_TM = '511'  " + CRLF
	cSql += "						AND D3_ESTORNO = ' '  " + CRLF
	cSql += "						AND D3_COD IN ( " + CRLF
	cSql += "						               SELECT B1_COD  " + CRLF
	cSql += "						  				 FROM " + RetSQLName('SB1') + "  " + CRLF
	cSql += "										WHERE SUBSTRING(B1_XLOTCOM,8,6) IN ( " +CRLF
	cSql += "										SELECT ZBC_PEDIDO FROM " + RetSQLName('ZBC') + " WHERE ZBC_CODIGO = '"+M->ZCC_CODIGO+"' AND D_E_L_E_T_ = ' ' )) " + CRLF
	...
	cSql += "										WHERE SUBSTRING(B1_XLOTCOM,8,6) IN ( " +CRLF
	cSql += "										SELECT ZBC_PEDIDO FROM " + RetSQLName('ZBC') + " WHERE ZBC_CODIGO = '"+M->ZCC_CODIGO+"' AND D_E_L_E_T_ = ' ' )) " + CRLF
	cSql += "										),0) [NASCIMENTO] " + CRLF
	cSql += "       FROM " + RetSQLName('ZBC') + " ZBC  " + CRLF
	cSql += "	  WHERE ZBC_FILIAL = '"+FWxFilial("ZBC")+"'  " + CRLF
	cSql += "	    AND ZBC_CODIGO = '"+M->ZCC_CODIGO+"' " + CRLF
	cSql += "		AND ZBC.D_E_L_E_T_ = ' '  " + CRLF
	cSql += "   GROUP BY ZBC_PEDIDO " + CRLF
	cSql += ") SELECT *, SALDO_B8-FATURADO-MORTE+NASCIMENTO TOTAL " + CRLF
	cSql += "    FROM DADOS " + CRLF
