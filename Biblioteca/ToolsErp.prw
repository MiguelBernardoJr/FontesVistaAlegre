...
User Function CALLCHGXNU() //Afterlogin()
    ...
    SetKey(K_ALT_S, {|| U_ToolsERP() })

Return Paramixb[5]
*/

/*{Protheus.doc} ToolsERP
...
User Function ToolsERP()
    ...
    If Type("OMAINWND") != "O"
        ...
        DEFINE WINDOW oMainWnd FROM 0,0 TO 0,0 TITLE "ToolsERP"
...
Static Function MainDevApp(lInit)
    ...
    Local aEmpExQr  := {}
    Local aFolder   := {'xQuery', 'Dicionário', 'Inspeção de Funções/Comandos', 'Linha de Comando','Html', 'File Explorer',"Monitor","Serviço","Error",'Conversor Base64' }
    ...
    FolderErro(oFol:aDialogs[9], oSize)        //--Folder Error
    FolderB64(oFol:aDialogs[10], oSize)        //--Folder Base64 Converter

    If lInit
        DEFINE MESSAGE BAR oMsgBar OF oDLG PROMPT "ToolsERP " COLOR RGB(116,116,116) FONT oFont
        DEFINE MSGITEM oMsgIt of oMsgBar PROMPT "Empresa/Filial: ["+cEmpAnt+"/"+cFilAnt+"] " SIZE 100  ACTION InitEmp(aEmpExQr, oMsgIt, .T., oDlg )
        DEFINE MSGITEM oMsgIt2 of oMsgBar PROMPT GetEnvServer() SIZE 100
        ACTIVATE MSDIALOG oDlg  ON INIT (lOk:= InitEmp(aEmpExQr, oMsgIt, .T., oDlg ))
    Else
...
Return  "TI" + Strzero(ThreadId(), 5) + __cSeqAliasTmp
...
Static Function PApQry2Run(cQuery, cAliasExqr, cAliasTst, oPnlQryI, np, oTime, oQtd)
    ...
    Begin Sequence
        ...
        Else
            ...
            dbUseArea( .T., "CTREECDX", cDir + cAliasExqr, cAliasExqr, .F., .F. )
            ...
            aQryBrw[np]:lColDrag	:= .F.
            aQryBrw[np]:lLineDrag	:= .F.
...
Static Function ProcCSV(cAliasCSV, cArquivo)
    ...
    For nx:= 1 to nFields
        cCab +=  aFields[nx, 1] + IIF( nx <> nFields , ";" , "" )
    Next
    ...
    While (cAliasCSV)->(! Eof())
        ...
        For nx:= 1 to nFields
            If aFields[nX][2] == "N"
                cLinha += Alltrim(StrTran(Padl(cValToChar((cAliasCSV)->(FieldGet(nx))) ,20), ".", ","))
            ElseIf aFields[nX][2] == "C"
                cLinha += Alltrim((cAliasCSV)->(Fieldget(nX)))
            ElseIf aFields[nX][2] == "D"
                cLinha += Alltrim(Dtoc((cAliasCSV)->(Fieldget(nX))))
            ElseIf aFields[nX][2] == "L"
                cLinha += If((cAliasCSV)->(Fieldget(nX)), "TRUE","FALSE")
            Else
                cLinha += Alltrim(cValToChar((cAliasCSV)->(FieldGet(nx))))
            EndIf

            cLinha += IIF( nx <> nFields , ";" , "" )
        Next
...
Static Function AtuSX3toDB(cAliasSX, oPnlDicI, np)
    ...
    cMsg := StartJob("u_ToolsX31",GetEnvServer(),.T.,SM0->M0_CODIGO,Alltrim(SM0->M0_CODFIL), cAliasSX)
...
User Function ToolsX31(cEmp, cFil, cAlias)
    Local cMsg:= ""
...
Static Function Tools2(cCMD, oOwner)
    ...
    If Empty(aCmdHst)
        ...
        aadd(aCmdHst, "cProg:= 'ToolsERP.PRW', Alert(VarInfo('Info',GetAPOInfo(cProg),,.F.))")
    EndIf
...
Static Function SaveJson()
    Local cFile := GetTempPath() + "ToolsERP.json"
    Local cConteudo := ""
...
Static Function DelThread(aLista, nAt, cEnvServer, cServerIP, nPortaTcp)
    ...
    If Alltrim(cEnvServer) == GetEnvserver() .and.  cServerIP  == GetServerIP() .and.  nPortaTcp == GetServerPort() .and. nThreadId == ThreadId()
