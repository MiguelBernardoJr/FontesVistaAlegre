...
// ---------+------------------------------+------------------------------------------------
 ...
 VA_TRTCOP
 C
 Parâmetro Customizado. Define a Cópia do Dia anterior considera calculado trato a trato, ou pela soma do trato do dia anterior
                ...
                // Busca quantidade de tratos do dia anterior
                    DbUseArea(.T., "TOPCONN", TCGenQry(,,;
                                          ...
                                          " left join " + RetSqlName("Z0G") + " Z0G ON " +;
                                                ...
                                                " and Z06.D_E_L_E_T_ = ' '" +;
                                            " group by Z06.Z06_FILIAL, Z06.Z06_DATA, Z06.Z06_VERSAO, Z06.Z06_LOTE, Z0I.Z0I_NOTMAN, Z0G.Z0G_ZERTRT" +;
                                            " )" +;
                                            " select Z06.*, QTDTRAT.QTD, QTDTRAT.Z0I_NOTMAN, QTDTRAT.Z0G_ZERTRT" +; 
                                                " from " + RetSqlName("Z06") + " Z06" +;
                                                " join QTDTRAT" +;
                    ...
                    */ 
                    ...
                    while !TMPZ06->(Eof())
                        ...
                        //endif
                        */
                        If (nTrtTotal / TMPZ06->QTD)  > 300 
                            If Z0G->Z0G_ZERTRT == "S"                            
                                If TMPZ06->Z06_TRATO == "1"
                                    nQtdTrato := 0
                                Else
                                    //if (++i == TMPZ06->QTD) 
                                    // Dividir trato do dia anterior pelos tratos restantes (sem considerar ) o primeiro
                                    nQtdRes := TMPZ05->Z05_KGMSDI / (TMPZ06->QTD -1) // 
                                    If GetMV("VA_AJUDAN") == "K"
                                        nQtdTrato := TMPZ05->Z05_KGMSDI + Z0G->Z0G_AJSTKG - nTotTrtClc
                                    ElseIf GetMV("VA_AJUDAN") == "P" // Se Ajuste for em Percentual (Z0G_PERAJU)
                                        //nQtdTrato := TMPZ05->Z05_KGMSDI + ((TMPZ05->Z05_KGMSDI * Z0G->Z0G_PERAJU ) / 100) - nTotTrtClc   
                                ...
                                EndIf
                            Else 
                                If TMPZ06->Z0G_ZERTRT == "S" // Quando no dia anterior zerar o trato, no dia seguinte, gerar normalmente 
                                    If GetMV("VA_AJUDAN") == "K"
                                        nQtdTrato := TMPZ05->Z05_KGMSDI + Z0G->Z0G_AJSTKG - nTotTrtClc
                                    ElseIf GetMV("VA_AJUDAN") == "P" // Se Ajuste for em Percentual (Z0G_PERAJU)
                                ...
                                Else
                                    ...
                                    ElseIf GetMV("VA_AJUDAN") == "P" // Se Ajuste for em Percentual (Z0G_PERAJU)
                                        //Anterior
                                        //nQtdTrato := TMPZ05->Z05_KGMSDI + ((TMPZ05->Z05_KGMSDI * Z0G->Z0G_PERAJU ) / 100) - nTotTrtClc   
                                        // COpiando trato a trato do dia anterior
                                        nQtdTrato := TMPZ06->Z06_KGMSTR + ((TMPZ06->Z06_KGMSTR * Z0G->Z0G_PERAJU ) / 100) //- nTotTrtClc
                                    EndIf
                                EndIf 
                            EndIf
                        Else// ((nTrtTotal / TMPZ06->QTD)) < 300
                            If (nTrtTotal / TMPZ06->QTD) < 300 
                                If Z0G->Z0G_ZERTRT == "S" .and. (nTrtTotal / (TMPZ06->QTD-1)) >= 300
                                    If TMPZ06->Z06_TRATO $ "1"
                                        nQtdTrato := 0
                                    Else
                                        nQtdTrato := (TMPZ05->Z05_KGMSDI +  ((TMPZ05->Z05_KGMSDI * Z0G->Z0G_PERAJU ) / 100)) / (TMPZ06->QTD-1)
                                    EndIf
                                ElseIf !Z0G->Z0G_ZERTRT == "S" .and. (nTrtTotal / (TMPZ06->QTD-1)) < 300 
                                    If TMPZ06->Z06_TRATO $ "1,4"
                                        nQtdTrato := 0
                                    Else
                                        nQtdTrato := (TMPZ05->Z05_KGMSDI +  ((TMPZ05->Z05_KGMSDI * Z0G->Z0G_PERAJU ) / 100)) / (TMPZ06->QTD-2)
                                    EndIf
                                EndIf 
                            EndIf
                        EndIf
        ...
        if LOTES->DIAS_NO_PLANO <= LOTES->MAIORDIAPL
            ...
            if !Empty(aChave := MaxVerTrat(LOTES->B8_LOTECTL, Z0R->Z0R_DATA))
                ...
                if !TMPZ05->(Eof())
                    ...
                    DbUseArea(.T., "TOPCONN", TCGenQry(,,;
                                              " with QTDTRAT as (" +;
                                                " select Z06.Z06_FILIAL, Z06.Z06_DATA, Z06.Z06_VERSAO, Z06.Z06_LOTE, count(*) QTD, Z0I.Z0I_NOTMAN, Z0G.Z0G_ZERTRT" +;
                                                 " from " + RetSqlName("Z06") + " Z06" +;
                                                 " left join " + RetSqlName("Z0I") + " Z0I ON " +;  
                                                  "     Z0I.Z0I_FILIAL = Z06.Z06_FILIAL" +; 
                                                  " AND Z0I.Z0I_DATA = Z06.Z06_DATA" +; 
                        ...
                        while !TMPZ06->(Eof())
                            ...
                            Else //(nTrtTotal / TMPZ06->QTD) < 300
                                If (nTrtTotal / TMPZ06->QTD) < 300 
                                    ...
                                    For n := 1 to TMPZ06->QTD
                                        If (nTrtTotal / (TMPZ06->QTD-1)) >= 300
                                            nTrt := nI
                                            Exit
                                        EndIf
                                    Next
                                    If Z0G->Z0G_ZERTRT == "S" .and. (nTrtTotal / (TMPZ06->QTD-nTrt)) >= 300
                                        If TMPZ06->Z06_TRATO $ "1"
                                            nQtdTrato := 0
                                        Else
                                            nQtdTrato := (TMPZ05->Z05_KGMSDI +  ((TMPZ05->Z05_KGMSDI * Z0G->Z0G_PERAJU ) / 100)) / (TMPZ06->QTD-1)
                                        EndIf
                                    ElseIf !Z0G->Z0G_ZERTRT == "S" .and. (nTrtTotal / (TMPZ06->QTD-2)) < 300 
                                        If TMPZ06->Z06_TRATO $ "1,4"
                                            nQtdTrato := 0
                                        Else
                                            nQtdTrato := (TMPZ05->Z05_KGMSDI +  ((TMPZ05->Z05_KGMSDI * Z0G->Z0G_PERAJU ) / 100)) / (TMPZ06->QTD-2)
                                ...
                                EndIf
                                //If !Mod(val(TMPZ06->Z06_TRATO),2) == 0
                                //    nQtdTrato := (TMPZ05->Z05_KGMSDI +  ((TMPZ05->Z05_KGMSDI * Z0G->Z0G_PERAJU ) / 100)) / round(TMPZ06->QTD / 2,0)
                                //Else
                                //    nQtdTrato := 0
                                //EndIf 
                            EndIf 
